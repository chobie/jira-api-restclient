...
class WalkerTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testFoundNoIssues()
	{
		...
		$this->api->search('test jql', 0, 5, 'description', array())->willReturn($search_response);
	...
	public function testDefaultPerPageUsed()
	{
		...
		$this->api->search('test jql', 0, 50, 'description', array())->willReturn($search_response);
	...
	public function testFoundTwoPagesOfIssues()
	{
		...
		$this->api->search('test jql', 0, 5, 'description', array())->willReturn($search_response1);

		// Incomplete 2nd page.
		$search_response2 = $this->generateSearchResponse('PRJ2', 2, 7);
		$this->api->search('test jql', 5, 5, 'description', array())->willReturn($search_response2);
	...
	{
		$this->api->search('test jql', 0, 5, 'description', array())->willThrow(new UnauthorizedException('Unauthorized'));
	...
	{
		$this->api->search('test jql', 0, 5, 'description', array())->willThrow(new \Exception('Anything'));
	...
	public function testUnauthorizedExceptionOnSecondPage()
	{
		...
		// Incomplete 2nd page.
		$this->api->search('test jql', 5, 5, 'description', array())->willThrow(new UnauthorizedException('Unauthorized'));
	...
	public function testAnyExceptionOnSecondPage()
	{
		...
		// Incomplete 2nd page.
		$this->api->search('test jql', 5, 5, 'description', array())->willThrow(new \Exception('Anything'));
	...
	public function testIssuesPassedThroughDelegate()
	{
		...
		$this->api->search('test jql', 0, 2, 'description', array())->willReturn($search_response);
